...
export default function RegisterPage() {
	...
	function handleInput(e: React.ChangeEvent<HTMLInputElement>) {
		...
		safeParseAsync(
			RegisterSchema.object[key as keyof RegisterData],
			updatedUserForm[key as keyof RegisterData]
		)
			.then((data) => {
			...
			>
				{statusMessage === "failed" ? (
					...
					<div className="text-xl text-green-500 font-serif flex flex-col gap-y-8">
						...
						<p>
							Ahora debe verificar su cuenta para poder hacer uso de su cuenta
						</p>
						<p>
							Para ello hemos envíado un enlace de confirmación a su correo
							electrónico
						</p>
					</div>
				)}
			...
			>
				<div className="w-[100%] flex justify-between items-center gap-x-2">
					...
					<div className="w-[10%] text-end">
						<span
							className={`${userForm.email.length > 0 ? "block" : "hidden"}`}
						>
				...
				<div className="w-[100%] flex justify-between items-center gap-x-2">
					...
					<div className="w-[10%] text-end">
						<span
							className={`${userForm.password.length > 0 ? "block" : "hidden"}`}
						>
				...
				<div className="w-[100%] flex justify-between items-center gap-x-2">
					...
					<div className="w-[10%] text-end">
						<span
							className={`${
								userForm.password2.length > 0 ? "block" : "hidden"
							}`}
						>
				...
				<div className="w-[100%] flex justify-between items-center gap-x-2">
					...
					<div className="w-[10%] text-end">
						<span
							className={`${userForm.username.length > 0 ? "block" : "hidden"}`}
						>
				...
				<div className="w-[100%] flex justify-between items-center gap-x-2">
					...
					<div className="w-[10%] text-end">
						<span
							className={`${
								userForm.gender === "Masculino" ||
								userForm.gender === "Femenino"
									? "block"
									: "hidden"
							}`}
						>

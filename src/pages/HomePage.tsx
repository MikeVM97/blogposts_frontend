	...
	useEffect(() => {
		const verifySession = async () => {
			...
			if (new Date() > expiration) {
				...
				// document.title = "BlogPosts | Dashboard"
			}
		}
		verifySession()
	}, [])
	...
	return (
		<div className={`relative bg-gray-300`}>
			<section className={`${loader ? "hidden" : "block"}`}>
				...
				<Posts
					postsOrdered={postsOrdered}
					user={user}
					setPostsOrdered={setPostsOrdered}
				/>
			</section>
			<div
				className={`${
					loader ? "block" : "hidden"
				} absolute top-0 left-0 w-screen h-screen p-4 bg-gray-200 flex flex-col justify-center items-center`}
			>
